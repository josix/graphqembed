...
def _get_perc_scores(scores, lengths):
    ...
    for i, length in enumerate(lengths):
        perc_scores.append(stats.percentileofscore(
            neg_scores[cum_sum:cum_sum + length], scores[i]))
        cum_sum += length
    return perc_scores
    ...
    for formula in test_queries:
        ...
        while offset < len(formula_queries):
            ...
            if hard_negatives:
                ...
                negatives = [random.choice(formula_queries[j].hard_neg_samples)
                             for j in xrange(offset, max_index)]
            else:
                lengths = [1 for j in range(offset, max_index)]
                negatives = [random.choice(formula_queries[j].neg_samples)
                             for j in xrange(offset, max_index)]
            offset += batch_size
            ...
            batch_scores = enc_dec.forward(formula,
                                           batch_queries +
                                           [b for i, b in enumerate(batch_queries)
...
def eval_perc_queries(test_queries, enc_dec, batch_size=1000, hard_negatives=False):
    ...
    for formula in test_queries:
        ...
        while offset < len(formula_queries):
            ...
            batch_queries = formula_queries[offset:max_index]
            if hard_negatives:
                lengths = [len(formula_queries[j].hard_neg_samples)
                           for j in range(offset, max_index)]
                negatives = [n for j in range(offset, max_index)
                             for n in formula_queries[j].hard_neg_samples]
            else:
                lengths = [len(formula_queries[j].neg_samples) for j in range(offset, max_index)]
                negatives = [n for j in range(offset, max_index)
                             for n in formula_queries[j].neg_samples]
            offset += batch_size
