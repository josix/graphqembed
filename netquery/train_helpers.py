...
def run_eval(model, queries, iteration, logger, by_type=False):
    ...
    for query_type in queries["one_neg"]:
        ...
        vals[query_type] = auc
        logger.info("{:s} val AUC: {:f} val perc {:f}; iteration: {:d}".format(
            query_type, auc, perc, iteration))
        if by_type:
            _print_by_rel(rel_aucs, logger)
        if "inter" in query_type:
            auc, rel_aucs = eval_auc_queries(
                queries["one_neg"][query_type], model, hard_negatives=True)
            perc = eval_perc_queries(queries["full_neg"][query_type], model, hard_negatives=True)
            logger.info(
                "Hard-{:s} val AUC: {:f} val perc {:f}; iteration: {:d}".format(query_type, auc, perc, iteration))
            if by_type:
                _print_by_rel(rel_aucs, logger)
...
def run_train(model, optimizer, train_queries, val_queries, test_queries, logger,
    ...
    for i in xrange(max_iter):
        ...
        if edge_conv:
            for query_type in train_queries:
                ...
                if "inter" in query_type:
                    loss += inter_weight * \
                        run_batch(train_queries[query_type], model, i, batch_size)
                    loss += inter_weight * \
                        run_batch(train_queries[query_type], model,
                                  i, batch_size, hard_negatives=True)
                else:
                    loss += path_weight * \
                        run_batch(train_queries[query_type], model, i, batch_size)
            if check_conv(vals):
    ...
    logger.info("Improvement from edge conv: {:f}".format(
        (np.mean(v.values()) - conv_test) / conv_test))
