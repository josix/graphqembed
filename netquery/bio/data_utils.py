    ...
    for i in range(2, 4):
        for kind in ["val", "test"]:
            if kind == "val":
                ...
                to_keep = 10000
            test_queries = load_queries_by_type(
                "/dfs/scratch0/nqe-bio/{:s}_queries_{:d}-split.pkl".format(kind, i), keep_graph=True)
            print "Loaded", i, kind
            for query_type in test_queries:
                test_queries[query_type] = [q for q in test_queries[query_type]
                                            if len(q.get_edges().intersection(deleted_edges)) > 0]
            ...
            pickle.dump(test_queries, open(
                "/dfs/scratch0/nqe-bio/{:s}_queries_{:d}-clean.pkl".format(kind, i), "wb"), protocol=pickle.HIGHEST_PROTOCOL)
...
def make_train_test_edge_data(data_dir):
    ...
    val_test_edge_queries = [Query(("1-chain", val_test_edges[i]),
                                   val_test_edge_negsamples[i], None, 100) for i in range(split_point)]
    ...
    pickle.dump([q.serialize() for q in val_queries], open(
        data_dir + "/val_edges.pkl", "w"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in test_queries], open(
        data_dir + "/test_edges.pkl", "w"), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    pickle.dump([q.serialize() for q in train_queries], open(
        data_dir + "/train_edges.pkl", "w"), protocol=pickle.HIGHEST_PROTOCOL)
...
# [random.choice(tuple(q[2]))]) for q in queries]
    ...
    pickle.dump(queries, open(file_name.split(".")[
                0] + "-split.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
...
def make_train_test_query_data(data_dir):
    ...
    pickle.dump([q.serialize() for q in queries_2], open(
        data_dir + "/train_queries_2.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in queries_3], open(
        data_dir + "/train_queries_3.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in t_queries_2[10000:]], open(
        data_dir + "/test_queries_2.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in t_queries_3[10000:]], open(
        data_dir + "/test_queries_3.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in t_queries_2[:10000]], open(
        data_dir + "/val_queries_2.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in t_queries_3[:10000]], open(
        data_dir + "/val_queries_3.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
